...
%o`, message, error3, detail);
  ...
  // src/controllers/load_more_controller.js
  ...
  // src/controllers/scroll_animation_controller.js
  var scroll_animation_controller_default = class extends Controller {
  ...
  function slugify(text) {
    return (text || "").toString().trim().toLowerCase().replace(/[\s]+/g, "-").replace(/[^\w\-]+/g, "").replace(/\-+/g, "-").replace(/^\-+|\-+$/g, "");
  }
  var toc_controller_default = class extends Controller {
    ...
    connect() {
      ...
      if (!this.contentEl || !this.hasLinksTarget)
        return;
      this.build();
      this.observe();
    }
    ...
    build() {
      ...
      this.linksTarget.innerHTML = items.map((it) => {
  ...
  application.register("scroll-animation", scroll_animation_controller_default);
  ...
  application2.register("load-more", load_more_controller_default);
  function wireLegalActions() {
    document.querySelectorAll("[data-legal-action]").forEach((el) => {
      if (el.dataset.legalWired === "1")
        return;
